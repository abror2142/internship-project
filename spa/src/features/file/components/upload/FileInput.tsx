...
import { uploadFile, deleteFile } from "../../../shared/utils/uploader";
...
function FileInput ({ setFileUrl, setFileName, setFileSize }) {
    ...
    const fetchSettings = async () => {
        try {
            ...
            const settingsObject = data?.settings.reduce((acc, cur) => {
                return { ...acc, ...cur };
            }, {});
    ...
    const getExtension = () => {
        if(file){
            ...
            const extension = extensions.filter(extension => extension.name == '.' + ext || extension.name == ext)[0];
            return extension;
        }
    }
        ...
        <div className="relative min-w-86 z-50">  
            {
                ...
                ? <div className="relative flex flex-col items-center justify-center w-full border-2 
                        border-gray-300 border-dashed rounded-lg bg-gray-50 dark:bg-gray-700 
                        dark:border-gray-600 dark:hover:border-gray-500 dark:text-dark-accent h-64">
                    <img 
                        src={getExtension()?.file_type.image} 
                        className="w-12"
                    />
                    <p>
                    ...
                    <div 
                        className="absolute top-0 right-0 text-red-500 p-1 mx-1"
